...
//	RestTemplate template;
	...
	public ResponseEntity<List<ProductDTO>> fetch(@RequestBody FetchDTO fetchDTO) {
		logger.info("Fetch request for location {}", fetchDTO.getLocation());
		List<ProductDTO> result = custService.fetch(fetchDTO);
		ResponseEntity<List<ProductDTO>> response = new ResponseEntity<List<ProductDTO>>(result, HttpStatus.OK);
		return response;
	}
